...
import numpy as np
import random
from utils import modifyArgsfile
...
class Data_loader():
    def __init__(self, args, phase="train"):
        ...
        if self.args.dataset=='ethucy':
            self.args.neighbor_dist = 10
            modifyArgsfile(self.args.config, 'neighbor_dist', 10)
            ...
            test_set = domains.index(args.test_set)
        if self.args.dataset=='madras':
            self.args.neighbor_dist = 5
            modifyArgsfile(self.args.config, 'neighbor_dist', 5)
            self.data_dirs = ['./data/madras/TopView_1B', './data/madras/TopView_2A',
                              './data/madras/TopView_2B', './data/madras/TopView_2C']
            domains = ['TopView_1B','TopView_2A','TopView_2B','TopView_2C']
            skip = [10, 10, 10, 10]
            test_set = domains.index(args.test_set)
        self.data_dir = './data'
        ...
        if phase == "train":
            ...
            self.test_dir = [self.data_dirs[test_set]]
            self.testskip=[skip[test_set]]

            self.test_data_file = os.path.join(self.args.model_filepath, f"test_trajectories_{args.test_set}.cpkl")
            self.test_batch_cache = os.path.join(self.args.model_filepath, f"test_batch_cache_{args.test_set}.cpkl")
    ...
    def dataPreprocess(self,setname):
        ...
        if setname=='train':
            data_index=self.get_data_index(frameped_dict,setname,True)
            ...
            test_index=data_index[:,int(data_index.shape[1]*val_fraction)+1:int(data_index.shape[1]*(val_fraction+test_fraction))]
            #indexes of the training set
            train_index = data_index[:,int(data_index.shape[1]*(val_fraction+test_fraction))+1:]
            ...
            testbatch = self.get_seq_from_index_balance(frameped_dict, pedtraject_dict, test_index, setname)
            ...
            data_index = self.get_data_index(frameped_dict, setname,False)
    ...
    def find_trajectory_fragment(self, trajectory,startframe,seq_length,skip):
        ...
        if self.args.dataset == 'madras':
            candidate_seq = candidate_seq[::10, :]
        try:
    ...
    def get_social_inputs_numpy(self, inputnodes):
        ...
        for pedi in range(num_Peds):
            ...
            for pedj in range(num_Peds):
                ...
                # invalid data index
                select_dist = (abs(relative_cord[:, 0]) > self.args.neighbor_dist) | (abs(relative_cord[:, 1]) > self.args.neighbor_dist)
